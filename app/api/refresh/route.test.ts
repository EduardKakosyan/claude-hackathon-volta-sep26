...
/**
 * The route's three stages, with the ingest functions replaced: what matters
 * here is the bearer check, the no-database answer, and that a conditions
 * failure — thrown or reported — never costs the status write its 200.
 */
...
vi.mock('@/lib/db/client', () => ({ getWriter: () => getWriter() }))
...
describe('GET /api/refresh', () => {
  beforeEach(() => {
    ...
    refreshLive.mockResolvedValue({ attemptedAt: 'x', today: '2026-09-13', sources: [], written: 0, omitted: 0 })
  ...
  it('runs seed, live, then conditions, and reports all three', async () => {
    const res = await GET(request('Bearer shh'))
